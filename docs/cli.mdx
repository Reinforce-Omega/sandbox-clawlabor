...
sandbox-agent api agents usage <AGENT> [--json] [--endpoint <URL>]
```
...
Show the Claude subscription usage for an agent, including the 5-hour and weekly usage windows plus plan information. Only the `claude` agent is supported.

| Option | Default | Description |
|--------|---------|-------------|
| `--json` | (off) | Print the raw JSON response instead of the formatted summary |
| `-e, --endpoint <URL>` | `http://127.0.0.1:2468` | Target server |
| `-t, --token <TOKEN>` | | Require/use bearer auth |
| `-n, --no-token` | | Disable auth |
